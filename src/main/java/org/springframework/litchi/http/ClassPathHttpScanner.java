...
public class ClassPathHttpScanner extends ClassPathBeanDefinitionScanner {
    ...
    public void registerFilters() {
        addIncludeFilter(new AnnotationTypeFilter(Host.class));
    ...
    public Set<BeanDefinitionHolder> doScan(String... basePackages) {
        ...
        if (beanDefinitions.isEmpty()) {
            logger.warn("No Http interface was found in '" + Arrays.toString(basePackages) + "' package. Please check your configuration.");
        } else {
    ...
    private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
        ...
        for (BeanDefinitionHolder holder : beanDefinitions) {
            ...
            logger.info("Creating HttpService with name '" + holder.getBeanName()
                    + "' and '" + definition.getBeanClassName() + "' httpInterface");
            definition.getConstructorArgumentValues().addGenericArgumentValue(definition.getBeanClassName());
            definition.setBeanClass(HttpServiceBuilder.class);
